...
#include "ADC_In.h"
    ...
    #define ADC_In_SetP4PinDriveMode(shift, mode)  \
    do { \
        ADC_In_PC =   (ADC_In_PC & \
                                (uint32)(~(uint32)(ADC_In_DRIVE_MODE_IND_MASK << \
                                (ADC_In_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (ADC_In_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define ADC_In_SetP4PinDriveMode(shift, mode)  \
        do { \
            ADC_In_USBIO_CTRL_REG = (ADC_In_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(ADC_In_DRIVE_MODE_IND_MASK << \
                                    (ADC_In_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (ADC_In_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		ADC_In_SetP4PinDriveMode(ADC_In__0__SHIFT, mode);
    }
#endif
...
void ADC_In_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  ADC_In_INTCFG & (uint32)(~(uint32)position);
    ADC_In_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
