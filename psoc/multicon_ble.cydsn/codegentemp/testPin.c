...
#include "testPin.h"
    ...
    #define testPin_SetP4PinDriveMode(shift, mode)  \
    do { \
        testPin_PC =   (testPin_PC & \
                                (uint32)(~(uint32)(testPin_DRIVE_MODE_IND_MASK << \
                                (testPin_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (testPin_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define testPin_SetP4PinDriveMode(shift, mode)  \
        do { \
            testPin_USBIO_CTRL_REG = (testPin_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(testPin_DRIVE_MODE_IND_MASK << \
                                    (testPin_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (testPin_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		testPin_SetP4PinDriveMode(testPin__0__SHIFT, mode);
    }
#endif
...
void testPin_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  testPin_INTCFG & (uint32)(~(uint32)position);
    testPin_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
